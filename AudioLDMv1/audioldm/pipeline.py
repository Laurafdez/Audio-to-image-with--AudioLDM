...
from utils import default_audioldm_config
...
def make_batch_for_image_to_audio(image, fbank=None, waveform=None,  batchsize=1):
    text = [image] * batchsize
    if batchsize < 1:
        print("Warning: Batchsize must be at least 1. Batchsize is set to .")
...
def image_to_audio(
    ...
    batch = make_batch_for_image_to_audio(image, batchsize=batchsize)
    ...
    with torch.no_grad():
        ...
        # Obtener el valor de cond_stage_config del diccionario config
        cond_stage_config = config["model"]["params"]["cond_stage_config"]

        ldm = LatentDiffusion(cond_stage_config=cond_stage_config, unet_config=config["model"]["params"]["unet_config"], timesteps=config["model"]["params"]["timesteps"])

        waveform = ldm.generate_image(
            [batch],
            unconditional_guidance_scale=guidance_scale,
            ddim_steps=ddim_steps,
            n_candidate_gen_per_text=n_candidate_gen_per_text,
