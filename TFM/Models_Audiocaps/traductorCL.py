...
class EmbeddingDataset(Dataset):
    ...
    def read_embeddings(self, archivo_ruta):
        ...
        clip_embeddings = torch.tensor(clip_embeddings, dtype=torch.float32).cuda()
        clap_embeddings = torch.tensor(clap_embeddings, dtype=torch.float32).cuda()
...
# Model
model = CLIptoCLAP(input_dim, hidden_dim, output_dim).cuda()
...
torch.save(model.state_dict(), 'modelCL256.pth')