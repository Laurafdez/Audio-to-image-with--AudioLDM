...
class EmbeddingDataset(Dataset):
    ...
    def read_embeddings(self, archivo_ruta):
        ...
        clip_embeddings = torch.tensor(eval(clip_embeddings_text), dtype=torch.float32).to('cuda:1')
        ...
        clap_embeddings = torch.tensor(eval(clap_embeddings_text), dtype=torch.float32).to('cuda:1')
...
class EmbeddingDataset1(Dataset):
    ...
    def read_embeddings(self, archivo_ruta):
        ...
        clip_embeddings = torch.tensor(clip_embeddings, dtype=torch.float32).to('cuda:1')
        clap_embeddings = torch.tensor(clap_embeddings, dtype=torch.float32).to('cuda:1')
...
class CLIptoCLAP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.dropout1 = nn.Dropout(p=0.5)  # Capa Dropout entre la primera y la segunda capa
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.dropout2 = nn.Dropout(p=0.5)  # Capa Dropout entre la segunda y la tercera capa
        self.fc3 = nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        x = F.normalize(x, p=2, dim=1)
        out = torch.relu(self.fc1(x.reshape(x.size(0), -1)))
        out = self.dropout1(out)  # Capa Dropout añadida
        out = torch.relu(self.fc2(out))
        out = self.dropout2(out)  # Capa Dropout añadida
        out = self.fc3(out)
...
class SimCLR(torch.nn.Module):
    ...
    def forward(self, emb_i, emb_j):
        ...
        """
        self.negatives_mask = self.negatives_mask.to('cuda:1')
        emb_i = emb_i.to('cuda:1')
        emb_j = emb_j.to('cuda:1')
...
for epoch in range(num_epochs):
    ...
    for clip_embedding, target_clap_embedding in train_dataloader:
        clip_embedding = clip_embedding.to('cuda:1')
        target_clap_embedding = target_clap_embedding.to('cuda:1')
...
torch.save(model.state_dict(), 'modelCL512mixdropaprend2.pth')