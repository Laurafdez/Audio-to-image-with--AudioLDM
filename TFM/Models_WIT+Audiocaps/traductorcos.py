...
for epoch in range(num_epochs):
    ...
    for clip_embedding, target_clap_embedding in train_dataloader:
        ...
        ones = torch.ones(len(target_clap_embedding)).to('cuda')

        predicted_clap_embedding = model(clip_embedding, target_clap_embedding)
        loss = criterion(predicted_clap_embedding, target_clap_embedding, ones)
...
torch.save(model.state_dict(), 'modelcos512mix.pth')